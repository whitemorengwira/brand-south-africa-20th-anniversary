...
# Project: Brand South Africa (Corporate · National Reputation Campaign)
# ------------------------------------------------------------------------------
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "Brand South Africa"
      Network     = "Corporate · National Reputation Campaign"
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Brand South Africa media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "brand_south_africa_20th_anniversary-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-brand-south-africa-20th-anniversary-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "brand_south_africa_20th_anniversary-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "brand_south_africa_20th_anniversary-post-production-policy"
  description = "Scoped S3 and KMS permissions for Brand South Africa post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/brand_south_africa_20th_anniversary"
